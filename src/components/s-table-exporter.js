...
export class STableExporter {
  ...
  getCellId(cell, rowOffset) {
    ...
    while (excelNumber > 0) {
      stack.push(
        String.fromCharCode(((excelNumber - 1) % 26) + "A".charCodeAt())
      );
  ...
  async handleCells(worksheet, table) {
    const { headerData, mainData, footerData, insertHeaderData } = table;
    // 计算插入到头部的数据
  ...
  formatData(data, type) {
    ...
    const { cells = [], columnStyle = [], rowStyle = [], rowLength = 0 } = data;
    if (rowLength === 0) {
  ...
  handleValue(worksheet, cellId, cell) {
    ...
    excelCell.value = !isEmptyObj(format) ? format : text;
  }
